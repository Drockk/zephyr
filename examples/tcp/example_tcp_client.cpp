 ...
 * @brief Standalone TCP client example with epoll and two-thread architecture
 *
 * Features:
 ...
 *
 * Compile: g++ -std=c++20 -pthread -o tcp_client example_tcp_client.cpp
 */
...
int connect_to_server(const char* ip, int port)
{
    ...
    // Connect in blocking mode first (simpler)
    ...
    // Now set non-blocking for async I/O
...
// ============================================================================
// TCP Client with epoll
// ============================================================================
class TcpClient
{
public:
    TcpClient(const char* server_ip, int server_port) : server_ip_(server_ip), server_port_(server_port) {}
    ...
    bool start()
    {
        socket_fd_ = connect_to_server(server_ip_, server_port_);
        if (socket_fd_ == -1) {
            return false;
        }
        ...
        business_thread_ = std::thread(&TcpClient::business_logic_loop, this);

        // Start I/O thread
        io_thread_ = std::thread(&TcpClient::io_loop, this);
...
private:
    ...
    void handle_socket_event(const epoll_event& ev)
    {
        if (ev.events & (EPOLLERR | EPOLLHUP)) {
            std::cerr << "[IO Thread] Connection error or closed by server\n";
            running_ = false;
            return;
        }
    ...
    void handle_read()
    {
        ...
        while (true) {
            ...
            if (bytes_read > 0) {
                ...
                incoming_queue_.push({data});
                std::cout << "[IO Thread] Received " << bytes_read << " bytes\n";

            } else if (bytes_read == 0) {
                // Server disconnected
                std::cout << "[IO Thread] Server closed connection\n";
                running_ = false;
                return;
            } else {
    ...
    void process_outgoing_messages()
    {
        ...
        while (outgoing_queue_.try_pop(msg)) {
            ...
            while (total_sent < to_send) {
                ssize_t sent = send(socket_fd_, msg.data.data() + total_sent, to_send - total_sent, MSG_NOSIGNAL);
                if (sent == -1) {
                    if (errno == EAGAIN || errno == EWOULDBLOCK) {
            ...
            std::cout << "[IO Thread] Sent " << total_sent << " bytes\n";
        }
    }
    ...
    void business_logic_loop()
    {
        ...
        std::vector<std::string> messages = {"Hello, Server!", "This is a test message.", "How are you doing?",
                                             "Zephyr networking example", "Goodbye!"};
        ...
        while (running_) {
            ...
            // If we've sent all messages and received responses, we can stop
            if (message_index >= static_cast<int>(messages.size())) {
                // Wait a bit more for any remaining responses
                std::this_thread::sleep_for(std::chrono::seconds(1));

                // Process any remaining responses
                while (incoming_queue_.try_pop(response)) {
                    process_response(response.data);
                }
